...
fn texture_asset_available(relative: &str) -> bool {
    #[cfg(target_arch = "wasm32")]
    {
        option_env!("SOLAR_BUNDLED_TEXTURES")
            .map(|list| {
                list.split(',')
...
fn spawn_body(
    ...
    let base_texture = def.texture.as_ref().and_then(|path| {
        if !texture_asset_available(path) {
            if def.texture.is_some() {
                warn!("Texture missing for '{}' — using color fallback", def.name);
            }
            return None;
        }
        ...
        Some(texture_handle(asset_server, texture_cache, path))
    });
